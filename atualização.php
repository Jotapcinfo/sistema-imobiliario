...
if(isset($_POST['submit'])){
   ...
   if(!empty($number)){
      ...
      if($verify_number->rowCount() > 0){
         $warning_msg[] = 'número já tomado!';
      }else{
         $update_number = $conn->prepare("UPDATE `users` SET number = ? WHERE id = ?");
         $update_number->execute([$number, $user_id]);
         $success_msg[] = 'número atualizado!';
      }
   }
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         $warning_msg[] = 'senha antiga não correspondida!';
      }elseif($new_pass != $c_pass){
         $warning_msg[] = 'confirme a senha não correspondida!';
      }else{
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <input type="password" name="c_pass" maxlength="20" placeholder="confirme sua nova senha" class="box">
